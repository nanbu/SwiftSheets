#!/usr/bin/env python3
"""Reads documents written by SwiftSheets with numbers-parser (MIT, the reference reader) and checks the values and
formulas (spec §11.3 "cross-implementation verification"). This and the self round trip run without Numbers.app;
`verify_with_numbers_app.py` is the same judgement passed by the application itself, and needs one.
...
if out:
    ...
    # Appendix B.18: the formula archives are generated by SwiftSheets, so the reference reader is the judge that
    # they are formulas and not only their last value. numbers-parser renders them without the leading "=".
    expect(t.cell(0, 6).formula == "SUM(B2:B3)", f"G1 formula {t.cell(0, 6).formula!r}")
    ...
    formulas = Document(str(out / "formulas.numbers")).sheets[0].tables[0]
    written = {formulas.cell(r, 3).value: formulas.cell(r, 4).formula for r in range(formulas.num_rows)}
    for source in ["1+2", "SUM(A1:A3)", "ROUND(SUM(A1:A3),2)", "$A$1+A$2+$A3", 'IF(A1>0,"y","n")',
                   "2^10", 'A1&"x"', "SUM({1,2;3,4})", "COUNTA(A:A)", "A1%", "-A1"]:
        got = written.get(source)
        expect(got is not None, f"{source}: numbers-parser found no formula")
        if got is not None:
            # numbers-parser spells a whole-column reference the way Numbers shows it, "A" rather than "A:A"
            expect(got.replace(" ", "") in (source.replace(" ", ""), source.replace(":A", "").replace(" ", "")),
                   f"{source} → {got!r}")
