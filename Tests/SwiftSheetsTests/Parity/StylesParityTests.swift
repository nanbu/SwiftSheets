...
@Suite struct StylesheetParityTests {
    ...
    @Test func noStylesheet() throws {
        let wb = try Workbook(data: minimalPackage(sheet: "<sheetData><row r=\"1\"><c r=\"A1\" s=\"3\"><v>1</v></c></row></sheetData>", styles: nil))
        #expect(wb.active["A1"].style == .default && wb.active["A1"].value == .integer(1))   // no styles part: every cell is default
    }
    ...
    @Test func simpleStyles() throws {
        ...
        ws["D9"].numberFormat = NumberFormat.number00; ws["D9"].protection = Protection(locked: true)
        ws["E1"].protection = Protection(hidden: true)
        let reg = StyleRegistry()
        for ref in ["A1", "A2", "A3", "A4", "A5", "D9", "E1"] { _ = reg.index(for: ws[ref].style) }   // same registration order as the reference
