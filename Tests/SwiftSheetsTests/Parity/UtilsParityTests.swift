...
@Suite struct UtilsCellParityTests {
    ...
    func absolute(_ coord: String, _ result: String) {
        #expect(CellReference.absolute(coord) == result)
    }

    // openpyxl: utils/tests/test_cell.py::test_column_interval
    @Test func columnInterval() {
        let expected = ["A", "B", "C", "D"]
        #expect(CellReference.columnLetters(from: "A", to: "D") == expected)
        #expect(CellReference.columnLetters(from: 1, to: 4) == expected)
    }
    ...
    @Test(arguments: [("j", 10), ("Jj", 270), ("JJj", 7030), ("A", 1), ("Z", 26), ("AA", 27), ("AZ", 52), ("BA", 53), ("BZ", 78), ("ZA", 677),
                      ("ZZ", 702), ("AAA", 703), ("AAZ", 728), ("ABC", 731), ("AZA", 1353), ("ZZA", 18253), ("ZZZ", 18278)])
    func columnIndex(_ column: String, _ idx: Int) {
        #expect(CellReference.columnIndex(column) == idx)
    }

    // openpyxl: utils/tests/test_cell.py::test_bad_column_index
    @Test(arguments: ["JJJJ", "", "$", "1"]) func badColumnIndex(_ column: String) {
        #expect(CellReference.columnIndex(column) == nil)
    }

    // openpyxl: utils/tests/test_cell.py::test_column_letter_boundries
    @Test(arguments: [0, 18729]) func columnLetterBoundaries(_ value: Int) {
        #expect(CellReference.columnLetter(validating: value) == nil)
    }
    ...
    @Test(arguments: [(18278, "ZZZ"), (7030, "JJJ"), (28, "AB"), (27, "AA"), (26, "Z")]) func columnLetter(_ value: Int, _ expected: String) {
        #expect(CellReference.columnLetter(value) == expected)
        #expect(CellReference.columnLetter(validating: value) == expected)
    }

    // openpyxl: utils/tests/test_cell.py::test_coordinate_tuple
    @Test func coordinateTuple() {
        let r = CellReference("D15")!
    ...
    @Test func invalidRange() {
        #expect(CellRange.splitSheetTitle("A1:E5") == nil)   // range_to_tuple requires a sheet name
    }
    ...
    func quoteSheetname(_ title: String, _ quoted: String) {
        #expect(CellReference.quoteSheetName(title) == quoted)
    }

    // openpyxl: utils/tests/test_cell.py::test_rows_from_range
    @Test func rowsFromRange() {
        let rows = CellRange("A1:D4")!.rows.map { $0.map(\.description) }
    ...
    @Test func colsFromRange() {
        let cols = CellRange("A1:D4")!.cols.map { $0.map(\.description) }
