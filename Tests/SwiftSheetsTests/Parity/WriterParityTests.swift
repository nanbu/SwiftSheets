...
@Suite struct WorksheetWriterParityTests {
    ...
    @Test func properties() {
        #expect(sheetXML(ws).contains("<sheetPr><outlinePr summaryBelow=\"1\" summaryRight=\"1\"/><pageSetUpPr/></sheetPr>"))
    }

    // openpyxl: worksheet/tests/test_writer.py::test_dimensions
    @Test func dimensions() {
        #expect(sheetXML(ws).contains("<dimension ref=\"A1:A1\"/>"))
    }

    // openpyxl: worksheet/tests/test_writer.py::test_format
    @Test func format() {
        #expect(sheetXML(ws).contains("<sheetFormatPr baseColWidth=\"8\" defaultRowHeight=\"15\"/>"))
    }

    // openpyxl: worksheet/tests/test_writer.py::test_views
    @Test func views() {
        #expect(sheetXML(ws).contains("<sheetViews><sheetView workbookViewId=\"0\"><selection activeCell=\"A1\" sqref=\"A1\"/></sheetView></sheetViews>"))
    }
    ...
    @Test func hyperlinks() {
        ws["A1"].value = "test"; ws["A1"].hyperlink = Hyperlink(target: "http://test.com")
    ...
    @Test func margins() {
        #expect(sheetXML(ws).contains("<pageMargins left=\"0.75\" right=\"0.75\" top=\"1\" bottom=\"1\" header=\"0.5\" footer=\"0.5\"/>"))
    }
    ...
    @Test func writeTail() {
        #expect(sheetXML(ws).hasSuffix("</sheetData><pageMargins left=\"0.75\" right=\"0.75\" top=\"1\" bottom=\"1\" header=\"0.5\" footer=\"0.5\"/></worksheet>"))
    }
    ...
    @Test func rowsSort() {
        for c in ["F1", "B1", "A1", "D1", "E1", "C1"] { ws[c].value = 1 }
    ...
    @Test func writeSheet() {
        ws["A10"].value = 15; ws["A10"].hyperlink = Hyperlink(target: "http://www.example.com")
...
@Suite struct ExcelWriterParityTests {
    ...
    @Test func worksheet() throws {
        let zip = try ZipArchive(data: try Workbook().save())
    ...
    @Test func modified() throws {
        ...
        #expect(wb.properties.modified == modified && back.properties.modified == modified)
    }
}
...
@Suite struct PackagingRelationshipParityTests {
    ...
    @Test func ctor() throws {
        ...
        let rel = p.rels[0]
        #expect(rel.id == "4" && rel.target == "drawings.xml" && rel.mode == "external" && rel.type.hasSuffix("/drawing"))
    }

    // openpyxl: packaging/tests/test_relationship.py::test_sequence
    @Test func sequence() throws {
        let wb = Workbook(); wb.createSheet()
        let rels = String(decoding: try ZipArchive(data: try wb.save()).read("xl/_rels/workbook.xml.rels"), as: UTF8.self)
