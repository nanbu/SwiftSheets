...
final class ContentParser: SAXHandler {
    ...
    /// The cell's text as ODF spells it — paragraphs, runs, links, the note — shared with the streaming reader
    /// so a cell reads the same by either road (spec Appendix B.40.2).
    ...
    func start(_ name: String, _ a: [String: String]) {
        ...
        case "table-cell", "covered-table-cell":
            ...
            cellText.reset()
        case "p":
            if validationMessage != nil { inValidationParagraph = true; messageText = "" }
        case "named-range":
    ...
    func end(_ name: String) {
        ...
        switch name {
        case "p":
            if validationMessage != nil, inValidationParagraph { inValidationParagraph = false; validationMessageText.append(messageText) }
        case "help-message", "error-message":
