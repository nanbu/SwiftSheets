...
// Package.swift
dependencies: [
    .package(url: "https://github.com/nanbu/SwiftSheets.git", from: "0.13.0")
],
targets: [
...
Status: **0.13.0** — all five formats are usable; the API may still change before 1.0. What changed in each release
...
| | |
|---|---|
| Whole workbook in memory | `Workbook` holds every cell: reckon on 100–200 bytes each, so a million cells is a few hundred megabytes — the working size, not the file size. `StreamingReader` walks a file of any format row by row instead — XLSX, ODS, Numbers and delimited text through one call — and `StreamingWriter` (XLSX) / `CSVStreamingWriter` write the same way: measured over a million cells ([the performance record](https://nanbu.github.io/SwiftSheets/performance.html) has the machine, the date and every other number), writing peaks at **11 MB** whatever the row count, and reading at **24 MB** for XLSX (most of it the shared-string table, which every reader must hold), **70 MB** for ODS and **85 MB** for Numbers (its string list, and the mapped file itself), against **214 MB** for the whole model. They carry values and formatting and nothing else — no merges, no notes, no preservation. A workbook of several sheets is parsed side by side when it is large enough — the same million cells in eight sheets: **257 MB** and 1.2 s against **145 MB** and 2.7 s one sheet at a time — and `ReadOptions.concurrency` caps or disables it, which also caps what it adds in memory. |
...
| `SheetXLSX` | `XLSXCodec` / `XLSMCodec`, `XLSXStreamingReader` / `StreamingWriter` | SheetCore |
...
| `SwiftSheets` | everything plus the facade: `Workbook(contentsOf:)`, `write(to:as:)`, `data(as:)`, `Workbook.convert`, and `StreamingReader` for any format | all of the above |
