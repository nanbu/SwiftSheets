...
| | |
|---|---|
| Whole workbook in memory | `Workbook` holds every cell: reckon on 100–200 bytes each, so a million cells is a few hundred megabytes — the working size, not the file size. `StreamingReader` / `StreamingWriter` (XLSX only) walk a file row by row instead: measured over a million cells, writing peaks at **10 MB** whatever the row count and reading at **23 MB** (most of it the shared-string table, which every reader must hold), against **221 MB** for the whole model. They carry values and formatting and nothing else — no merges, no notes, no preservation. |
