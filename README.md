...
// Package.swift
dependencies: [
    .package(url: "https://github.com/nanbu/SwiftSheets.git", from: "0.17.1")
],
targets: [
...
Status: **0.17.1** — all five formats are usable; the API may still change before 1.0. What changed in each release
...
| | |
|---|---|
| Whole workbook in memory | `Workbook` holds every cell: reckon on 100–200 bytes each, so a million cells is a few hundred megabytes — the working size, not the file size. `StreamingReader` walks a file of any format row by row instead — XLSX, ODS, Numbers and delimited text through one call — and `StreamingWriter` writes the same way in every format: measured at 100 columns × 10,000 rows, a million cells ([the performance record](https://nanbu.github.io/SwiftSheets/performance.html) has the machine, the date, every other number, and the same operations at 100,000 rows — ten million cells), writing peaks at **10 MB** for XLSX and **22 MB** for ODS (its one part puts the styles before the rows, so the rows wait on disk until the end) whatever the row count, and at **25 MB** for Numbers (its string list, which grows with the distinct strings: 62 MB at ten million cells); reading peaks at **19 MB** for XLSX (most of it the shared-string table, which every reader must hold), **53 MB** for Numbers (its string list, and the mapped file itself) and **228 MB** for ODS — on a wide sheet the ODS row-by-row reader holds nearly as much as the whole model, a known gap on [the plan](docs/plan.md) — against **215 MB** for the whole model. They carry values and formatting and nothing else — no merges, no notes, no preservation. A workbook of several sheets is parsed side by side when it is large enough — the same million cells in eight sheets: **249 MB** and 1.1 s against **141 MB** and 2.6 s one sheet at a time — and `ReadOptions.concurrency` caps or disables it, which also caps what it adds in memory. |
